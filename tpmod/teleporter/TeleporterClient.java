...
import tpmod.location.PortalPositionClient;

public class TeleporterClient
{
	private final World worldServerInstance;
	...
	public TeleporterClient(World world)
	{
			...
			for (int l = -2; l <= 2; ++l)
			{
				for (int i1 = -2; i1 <= 2; ++i1)
				{
					for (int j1 = -1; j1 < 3; ++j1)
					{
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.worldServerInstance.setBlock(k1, l1, i2, (Block) (flag ? Blocks.obsidian : 0));
					}
				}
			}

			par1Entity.setLocationAndAngles((double)i + 1, (double)j, (double)k + 1, par1Entity.rotationYaw, 0.0F);
	...
	public boolean placeInExistingPortal(Entity entity, double par2, double par4, double par6, float par8)
	{
		...
		if (d3 >= 0.0D)
		{
			if (flag)
			{
				this.destinationCoordinateCache.add(j1, new PortalPositionClient(this, i, j, k, this.worldServerInstance.getTotalWorldTime()));
			...
			if (j2 > -1)
			{
				...
				x += (double)((float)k3 * f1 + f2 * (float)i3);
				z += (double)((float)l3 * f1 + f2 * (float)j3);
				float f3 = 0.0F;
				float f4 = 0.0F;
				float f5 = 0.0F;
				float f6 = 0.0F;
			...
			entity.setLocationAndAngles(x + 2, y, z, entity.rotationYaw, entity.rotationPitch);
			return true;
		}
		else
		{
			...
			for (k2 = -1; k2 <= 1; ++k2)
			{
				for (i3 = 1; i3 < 3; ++i3)
				{
					for (l2 = -1; l2 < 3; ++l2)
					{
						...
						i4 = j2 + (i3 - 1) * l5 - k2 * k5;
						flag = l2 < 0;
						this.worldServerInstance.setBlock(k3, j3, i4, (Block) (flag ? Blocks.obsidian : 0));
					}
				}
			}
		}
